...
async def _handle_entity_relation_summary(
    ...
    if isinstance(entity_or_relation_name, tuple):
        if len(entity_or_relation_name) == 3:
            ...
            entity_name = f"关系：从 {src_id} 到 {tgt_id}，关键词：{keyword}"
        elif len(entity_or_relation_name) == 2:
            src_id, tgt_id = entity_or_relation_name
            entity_name = f"关系：从 {src_id} 到 {tgt_id}"
        else:
            entity_name = "未知关系"
    else:
